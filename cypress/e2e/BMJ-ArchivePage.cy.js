const journals = ["https://bmjoncology.bmj.com/content/3/1"];

describe("Archive Page Inspect", () => {
  it.only("Find Article URL", () => {
    var ArticleUrl = [];
    const ArticleUrlId = "cypress/downloads/CONTENT/BMJoncology/Url.csv";
    journals.forEach((journal) => {
      ...
      cy.get(".issue-toc")
        .find("a")
        .each(($ele) => {
          ArticleUrl.push($ele.attr("href"));
          cy.writeFile(ArticleUrlId, ArticleUrl);
        });
    });
  });

  it("Find Article URL By Category", () => {
    var Analysis = [];
    var Editorial = [];
    var Originalarticle = [];
    ...
    journals.forEach((journal) => {
      ...
      cy.get("body").then(($body) => {
        if ($body.find("#Analysis").length > 0) {
          cy.get(".issue-toc-section-8203editorial8203 > ul > li")
            .find("a")
            .each(($ele) => {
        ...
        if ($body.find("#Editorial").length > 0) {
          cy.get(".issue-toc-section-8203editorial > ul > li")
            .find("a")
            .each(($ele) => {
        ...
        if ($body.find("#Originalresearch").length > 0) {
          cy.get(".issue-toc-section-8203original-research > ul > li")
            .find("a")
            .each(($ele) => {
        ...
        if ($body.find("#Regulatoryandpolicycorner").length > 0) {
          cy.get(".issue-toc-section-regulatory-and-policy-corner > ul > li")
            ...
            .each(($ele) => {
              ...
              cy.writeFile(
                RegulatoryandpolicycornerId,
                Regulatoryandpolicycorner
              );
            });
        } else {
        ...
        if ($body.find("#Review").length > 0) {
          cy.get(".issue-toc-section-8203review > ul > li")
            .find("a")
            .each(($ele) => {
        ...
        if ($body.find("#Perspective").length > 0) {
          cy.get(".issue-toc-section-8203perspective > ul > li")
            .find("a")
            .each(($ele) => {
