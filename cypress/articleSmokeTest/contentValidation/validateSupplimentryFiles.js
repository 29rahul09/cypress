    ...
    if ($body.find('[data-testid="overview"]').length > 0) {
      cy.get('[data-testid="overview-list"] a').then(($anchor) => {
        ...
        if (overviewSupplementary && bodySupplementary) {
                ...
                .then((response) => {
                  if (response.status !== 200) {
                    missingSupplementary.push(
                      `Error in Supplementary Files ==> /content${articleUrl} ==> ${url}`
                    );
                  }
                })
                .then(() => {
          ...
          missingSupplementary.push(
            `NO Supplementary in Body ==> /content${articleUrl}`
          );
          ...
          missingSupplementary.push(
            `NO Supplementary in Overview ==> /content${articleUrl}`
          );
          ...
          nonSupplementary.push(`NO Supplementary Files ==> /content${articleUrl}`);
